...
## 1. Scope
This document explains every relevant aspect of `py/aiwaf/core/keyword_fallback.py`, including:
...
Primary module:
- `py/aiwaf/core/keyword_fallback.py`

Primary integration point:
- `py/aiwaf/django/storage.py` (`ModelKeywordStore` fallback path)
...
- `py/aiwaf/django/fallback_keywords.json`
...
Implementation summary (`py/aiwaf/core/keyword_fallback.py`):
...
File: `py/aiwaf/django/storage.py`
...
No file-lock strategy is used here, unlike CSV helpers in `py/aiwaf/core/storage_csv.py` that implement lock/retry patterns.
...
- Core module is exported by `py/aiwaf/core/__init__.py`, making it accessible for shared usage.
