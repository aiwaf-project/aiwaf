...
class ExemptionDecoratorsDjangoTestCase(AIWAFTestCase):
    ...
    def test_decorated_views_still_execute(self):
        request = self.create_request()

        @aiwaf_exempt
        def fully_exempt(_request):
            return "exempt"

        @aiwaf_exempt_from("logging")
        def partly_exempt(_request):
            return "partial"

        @aiwaf_require_protection("logging")
        def required(_request):
            return "required"

        self.assertEqual(fully_exempt(request), "exempt")
        self.assertEqual(partly_exempt(request), "partial")
        self.assertEqual(required(request), "required")
