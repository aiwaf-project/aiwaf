...
from tests.django.base_test import AIWAFTrainerTestCase
from aiwaf.django.trainer import get_legitimate_keywords
...
class RouteKeywordExtractionTestCase(AIWAFTrainerTestCase):
    ...
    def _patch_learning_deps(self, store, path_exists):
        stack = ExitStack()
        stack.enter_context(patch("aiwaf.django.middleware.get_keyword_store", return_value=store))
        stack.enter_context(patch("aiwaf.django.middleware.is_middleware_disabled", return_value=False))
        stack.enter_context(patch("aiwaf.django.middleware.is_exempt", return_value=False))
        stack.enter_context(patch("aiwaf.django.middleware.is_ip_exempted", return_value=False))
        stack.enter_context(patch("aiwaf.django.middleware.BlacklistManager.is_blocked", return_value=False))
        stack.enter_context(patch("aiwaf.django.middleware.path_exists_in_django", return_value=path_exists))
        return stack
