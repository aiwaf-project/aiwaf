...
from aiwaf.django.utils import get_ip
from aiwaf.django.middleware_logger import AIWAFLoggerMiddleware
...
class RustBackendToggleTests(AIWAFMiddlewareTestCase):
    ...
    def test_header_validation_uses_rust_when_enabled(self):
        ...
        with patch("aiwaf.django.middleware.rust_available", return_value=True), patch(
            "aiwaf.django.middleware.rust_validate_headers", return_value="Rust says no"
        ), patch.object(
...
class RustFallbackTests(AIWAFMiddlewareTestCase):
    ...
    def test_header_validation_falls_back_when_rust_unavailable(self):
        ...
        with patch("aiwaf.django.middleware.rust_available", return_value=False), \
             patch("aiwaf.django.middleware.rust_validate_headers") as rust_fn:
            middleware = HeaderValidationMiddleware(self.mock_get_response)
            response = middleware.process_request(request)
    ...
    def test_ai_anomaly_prefers_rust_behavior_analysis(self):
        ...
        with patch("aiwaf.django.middleware.rust_available", return_value=True), \
             patch("aiwaf.django.middleware.path_exists_in_django", return_value=False), \
             patch("aiwaf.django.middleware.is_exempt_path", return_value=False), \
             patch("aiwaf.django.middleware.rust_analyze_recent_behavior", return_value=stats) as rust_fn, \
    ...
    def test_ai_anomaly_falls_back_when_rust_missing(self):
        ...
        with patch("aiwaf.django.middleware.rust_available", return_value=False), \
             patch("aiwaf.django.middleware.path_exists_in_django", return_value=False), \
             patch("aiwaf.django.middleware.is_exempt_path", return_value=False), \
             patch("aiwaf.django.middleware.rust_analyze_recent_behavior", return_value=None) as rust_fn, \
