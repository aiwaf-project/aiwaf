...
def test_logging_formats():
    ...
    try:
        ...
        from aiwaf.flask import register_aiwaf_middlewares
...
def test_log_analysis():
    ...
    try:
        from aiwaf.flask.logging_middleware import analyze_access_logs
...
def test_logging_factory_block_event_and_json_analysis(tmp_path):
    from flask import Flask
    from aiwaf.flask import logging_middleware

    app = Flask(__name__)
    middleware = logging_middleware.get_logging_middleware()
    middleware.aiwaf_log_file = str(tmp_path / "aiwaf.log")
    with app.test_request_context("/admin", headers={"X-Forwarded-For": "203.0.113.70"}):
        middleware.mark_request_blocked("test")
        middleware._log_aiwaf_event(None)
    assert "test" in (tmp_path / "aiwaf.log").read_text(encoding="utf-8")

    log = tmp_path / "events.jsonl"
    log.write_text(json.dumps({"status_code": 403, "ip": "203.0.113.70"}) + "\n", encoding="utf-8")
    stats = {"total_requests": 0, "status_codes": {}, "ips": {}, "blocked_requests": 0}
    logging_middleware._analyze_json_logs(log, stats)
