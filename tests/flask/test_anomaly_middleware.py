...
def test_anomaly_helper_features_context_and_stats():
    ...
    middleware.app = app
    with app.test_request_context("/.env?cmd=exec"):
        assert middleware._is_malicious_context(request, ".env")
        features = middleware._calculate_features(request, "203.0.113.110", 0.1)
        assert len(features) > 0
    assert middleware.get_stats()["malicious_keywords"] > 0
...
def test_ai_anomaly_middleware():
    ...
    if ai_anomaly_middleware:
        ...
        print("  AI Anomaly middleware not found (but this is expected without direct access)")
    ...
    print("- ML model availability (model.json)")
