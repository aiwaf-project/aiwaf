...
def test_feature_toggles_reset_and_security_view():
    from aiwaf.core.runtime_config import AIWAFConfig

    config = AIWAFConfig()
    config.disable_feature("header_validation")
    assert not config.is_enabled("header_validation")
    config.enable_feature("header_validation")
    assert config.is_enabled("header_validation")
    assert "max_header_length" in config.get_security_config()
    config.set("storage.backend", "bad")
