...
describe('AIWAF Adonis middleware', () => {
  it('implements the complete Express response contract', async () => {
    jest.resetModules();
    jest.doMock('../lib/wafMiddleware', () => () => (_req, res) => {
      const initialStatus = res.statusCode;
      res.statusCode = initialStatus + 1;
      res.send('adapter-send');
    });
    const createMiddleware = require('../lib/adonisMiddleware');
    const ctx = createMockCtx();
    await createMiddleware()(ctx, jest.fn());
    expect(ctx.response.statusCode).toBe(201);
    expect(ctx.response.body).toBe('adapter-send');
