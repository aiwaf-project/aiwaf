...
describe('exemptions DB integration', () => {
  ...
  it('initializes and clears both exemption tables', async () => {
    await exemptionStore.initialize();
    await exemptionStore.addIp('198.51.120.21', 'clear-test');
    await exemptionStore.addPath('/clear-test', 'clear-test');
    const deleted = await exemptionStore.clear();
