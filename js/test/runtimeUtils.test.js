...
describe('runtime utils parity helpers', () => {
  ...
  test('extracts request IPs, allowlists, and redacted extended details', () => {
    const req = {
      method: 'POST',
      url: '/submit',
      headers: {
        'x-forwarded-for': '::ffff:203.0.113.7, 10.0.0.1',
        authorization: 'secret',
        accept: 'application/json'
      }
    };
    expect(runtimeUtils.getRequestIp(req)).toBe('203.0.113.7');
    expect(runtimeUtils.ipInAllowlist('203.0.113.7', ['203.0.113.*'])).toBe(true);
    const details = runtimeUtils.extractExtendedRequestInfo(req, { enabled: true });
    expect(details.ip).toBe('203.0.113.7');
    expect(details.headers.authorization).toBe('[redacted]');
    expect(runtimeUtils.extractExtendedRequestInfo(req, { enabled: false })).toBeNull();
  });
