...
describe('dynamicKeywordStore CSV fallback', () => {
  ...
  it('initializes, adds, replaces, and removes keywords', async () => {
    const store = require('../lib/dynamicKeywordStore');
    await store.initialize();
    await store.add('Probe', 4);
    await store.add('Probe', 7);
    expect((await store.list()).find(row => row.keyword === 'probe').count).toBe(7);
