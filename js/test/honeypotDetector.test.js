...
describe('honeypotDetector method and timing policies', () => {
  ...
  it('evaluates an obvious POST-only probe path', () => {
    honeypotDetector.init({ AIWAF_METHOD_POLICY_ENABLED: true });
    const result = honeypotDetector.evaluate({ method: 'GET' }, '198.51.100.12', '/account/submit/');
    expect(result).toEqual(expect.objectContaining({ triggered: true, statusCode: 405 }));
  });
});
